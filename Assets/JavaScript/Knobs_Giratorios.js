function calcularGrados(e, ObjetoX, ObjetoY, ancho_O_altoKnob) {
  ...
  if (PosXMouseWithKnobAsReference >= 0 && PosYMouseWithKnobAsReference >= 0) {
    grados =
      (Math.atan(PosYMouseWithKnobAsReference / PosXMouseWithKnobAsReference) *
        180) /
      Math.PI;
  } else if (
    PosXMouseWithKnobAsReference < 0 &&
    PosYMouseWithKnobAsReference >= 0
  ) {
    grados =
      (Math.atan(
        Math.abs(PosXMouseWithKnobAsReference / PosYMouseWithKnobAsReference)
      ) *
        180) /
        Math.PI +
      90;
    ...
    grados =
      (Math.atan(
        Math.abs(PosYMouseWithKnobAsReference / PosXMouseWithKnobAsReference)
      ) *
        180) /
        Math.PI +
      180;
...
function gradosCSSaValorKnob(gradosCSS, limiteInferior, limiteSuperior) {
  return (
    ((gradosCSS + 135) * (limiteSuperior - limiteInferior)) / 270 +
    limiteInferior
  );
}
...
function ValorKnobAGradosCSS(valorKnob, limiteInferior, limiteSuperior) {
  return (
    ((valorKnob - limiteInferior) * 270) / (limiteSuperior - limiteInferior) -
    135
  );
}
...
function insertaKnobsEn(
  ...
  let altoContenedoresDeKnobs =
    100 / Math.ceil(cantidadKnobs / cantidadKnobsPorFila) - 1 + "%";
  ...
  for (let i = 0; i < cantidadKnobs; i++) {
    ...
    let crearMascaraDeArrastreParaMoverKnob = () => {
      ...
      funcionesRotate[i] = (e) => {
        ...
        let rotacion = calcularGrados(
          e,
          datosGeometricosKnob[i].left,
          datosGeometricosKnob[i].top,
          datosGeometricosKnob[i].width
        );
        Knobs[i].style.transform = `rotate(${rotacion}deg)`;
        knobsValues[i] = gradosCSSaValorKnob(
          rotacion,
          limitesInferiores[i],
          limitesSuperiores[i]
        );

        contenedorDeKnobs.dispatchEvent(
      ...
      eventosMouseDownIDs[i] = delegarEvento(
        "mousemove",
        mascarasDeArrastre[i],
        funcionesRotate[i]
      );

      eventosMouseUpIDs[i] = delegarEvento(
        "mouseup",
        mascarasDeArrastre[i],
        function () {
    ...
    delegarEvento(
      "mousedown",
      indicadores_knobs[i],
      crearMascaraDeArrastreParaMoverKnob
    );
  ...
  function guardarValores(valores) {
    for (let i = 0; i < cantidadKnobs; i++) {
      if (valores[i] !== undefined) {
        Knobs[i].style.transform = `rotate(${ValorKnobAGradosCSS(
          valores[i],
          limitesInferiores[i],
          limitesSuperiores[i]
        )}deg)`;
